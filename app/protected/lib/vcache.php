...
class vcache
{
    ...
    public static function instance($type = 'memcache', $force_replace = FALSE)
    {
        ...
        {
            if(get_cfg_var("erhuo.runmode") === 'online' && class_exists('Memcache') && $type == 'memcache')
            {
                $mem = new Memcache();
                if($mem->addServer('127.0.0.1', 12321)) self::$_mem = $mem;
    ...
    {
        if(self::$_mem) {
            return self::$_mem->set($key, $val, FALSE, $expires);
        }
        $expires_since = $_SERVER['REQUEST_TIME'] + $expires;
        return file_put_contents($this->_filename($key), '<?php die();?>,'.$expires_since.','.base64_encode(serialize($val)));
    }
    ...
    {
        if(self::$_mem) return self::$_mem->delete($key);
