...
class user_controller extends general_controller
{
    public function action_login()
    {
        ...
        if ($username && $user = $user_model->find_user_by_account($username, $password)) {
            $user_model->stay_login($user['user_id'], $user['password'], $client_ip);//保持登录
            $user_model->set_logined_info($client_ip, $user['user_id'], $user['username'], $user['avatar']);
            $res = array('status' => 'success', 'user_id' => $user['user_id'], 'redirect' => isset($_SESSION['REDIRECT']) ? $_SESSION['REDIRECT'] : null);
            unset($_SESSION['REDIRECT']);
        } else {
            $error_model->incr_err($client_ip);
            $res = array('status' => 'error', 'msg' => '账号或密码错误');
        }
        ...
        if (TRUE === $verifier) {
            if ($user_id = $user_model->register($data)) {
                ...
                if (isset($_SESSION['OAUTH'])) {
                    $profile = [
                        'nickname' => $_SESSION['OAUTH']['NICKNAME'],
                        'gender' => $_SESSION['OAUTH']['GENDER'],
                    ];
                    $user_profile_model = new user_profile_model();
                    $user_profile_model->update(['user_id' => $user_id], $profile);
                }
                $token_model = new token_model();
                $token = $token_model->generate(['user_id' => $user_id]);
                $res = array('status' => 'success', 'user_id' => $user_id, 'token' => $token);
            } else {
        ...
        switch (request('field')) {
            case 'avatar':
                try {
                    ...
                    $key = $qiniu_model->put64($streams);
                    $user_model = new user_model();
                    $user_model->update(array('user_id' => $user_id), array('avatar' => $GLOBALS['cfg']['image_host'].DS.$key));
    ...
    public function action_set_mobile()
    {
        $code = request('code');
        $mobile = request('mobile');

        $res = phone::verify_code($mobile, $code);
        if (true !== $res) {
            $this->r(false, "验证码错误");
        }
        $user_id = $this->is_logined();
        $user_model = new user_model();
        $user_model->update(['user_id' => $user_id], ['mobile' => $mobile]);
        $this->r(true, '设置成功');
    }
}