...
class consignee_controller extends general_controller
{
    ...
    public function action_detail()
    {
        ...
        $id = (int)request('id', 0);
        $consignee_model = new user_consignee_model();
        if ($consignee = $consignee_model->find(array('id' => $id, 'user_id' => $user_id))) {
            echo json_encode(array('status' => 'success', 'data' => $consignee));
        } else {
            echo json_encode(array('status' => 'error'));
        }
    }
    ...
    public function action_save()
    {
        ...
        (
            ...
            'room' => trim(strip_tags(request('room', '', 'post'))),
