...
class oauth_controller extends general_controller
{
    ...
    {
        $oauthKey = request("oauthKey", null, 'post');
        if (empty($_SESSION['OAUTH']['KEY']) && is_null($oauthKey)) die('Bad Request');
        ...
        if (!$oauth_model->find($row)) {
            $row['oauth_key'] = isset($_SESSION['OAUTH']['KEY']) ? $_SESSION['OAUTH']['KEY'] : $oauthKey;
            $row['unionid'] = isset($_SESSION['OAUTH']['UNIONID']) ? $_SESSION['OAUTH']['UNIONID'] : '';
