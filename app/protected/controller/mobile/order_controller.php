...
class order_controller extends general_controller
{
    public function action_confirm()
    {
        ...
        if($cart)
        {
            ...
            $this->school_list = $vcache->school_model('get_school_list');
            $this->consignee_list = $consignee_model->get_user_consignee_list($user_id);
            $this->shipping_list = $vcache->shipping_method_model('indexed_list');
            $this->user = $vcache->user_model('find', array('id' => $user_id));
    ...
    public function action_view()
    {
        ...
        if($order = $order_model->find(array('order_id' => $order_id, 'user_id' => $user_id)))
        {
            ...
            $goods_model = new goods_model();
            $good_id_list = implode(",",array_column($this->goods_list,"goods_id"));
            $this->recommend_goods_list = $goods_model->get_related($good_id_list,6);
            ...
            $this->order_full_cut = $order_full_cut_model->getByOrderId($order_id);
            ...
            elseif($order['order_status'] == 3)
            {
                ...
                if($shipping = $shipping_model->find($condition, 'dateline DESC'))
                {
                    ...
                    if(!$this->countdown) $order_model->update($condition, array('order_status' => 4, 'finish_date' => time()));
