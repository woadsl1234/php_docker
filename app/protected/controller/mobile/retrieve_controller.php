        ...
        switch($this->step)
        {
            ...
            case 2:
                ...
                if($user = $user_model->find(array('username' => trim(request('username', '', 'post'))), null, 'email, password'))
                {
                    ...
                    $email = substr($user['email'], 0, 1) . '***' . substr($user['email'], $pos - 2, 2) . substr($user['email'], $pos);
                    $this->email = $email;
            ...
            case 4:
                ...
                if(empty($_SESSION['RETRIEVE_PWD']['UID'])) jump(url('mobile/main', '400'));
                ...
                if(TRUE === $verifier)
                {
                    ...
                    $_SESSION['USER']['USER_ID'] = $_SESSION['RETRIEVE_PWD']['UID'];
                    unset($_SESSION['RETRIEVE_PWD']);
                    $this->prompt('success', '重置成功，即将跳转至用户中心', url('mobile/user', 'index'), 3);
                }
                else
                {
