...
use app\model\aftersales_model;
use app\model\goods_instead_sell;
...
use app\model\user_favorite_model;
use app\model\user_model;
use app\model\user_profile_model;
use app\model\user_record_model;
...
class user_controller extends general_controller
{
    public function action_index()
    {
        if (request('step') == 'search') {
            ...
            $kw = trim(request('kw', ''));
            if ($kw != '') {
                $types = ['username', 'email', 'mobile'];
                $where_field = $types[request('type', 0)];
            ...
            if ($total[0]['count'] > 0) {
                ...
                $sql = "SELECT a.user_id, a.username, a.email,a.mobile , a.status, a.email_status,
    ...
    public function action_revise_account()
    {
        ...
        if (TRUE === $verifier) {
            ...
            if ($account_model->update_account($user_id, $data, $data['cause'])) {
