...
use app\model\wechat_templates_model;
...
class order_controller extends general_controller
{
            ...
            if ($keyword != '') {
                switch (request('search_type')) {
                    case 'order_id':
                        ...
                        $binds[':order_id'] = "%" . $keyword . "%";
                        break;
                    case 'receiver':
                        $where .= ' AND b.receiver like :receiver';
                        $binds[':receiver'] = "%" . $keyword . "%";
                        break;
                    case 'mobile':
                        $where .= ' AND b.mobile like :mobile';
                        $binds[':mobile'] = "%" . $keyword . "%";
                        break;
                    case 'goods_name':
                        $sql = "select order_id from {$GLOBALS['mysql']['MYSQL_DB_TABLE_PRE']}order_goods where goods_name like '%$keyword%'";
                        $order_goods_list = $order_model->query($sql);
                        if (!empty($order_goods_list)) {
                            $order_goods_ids = [];
                            foreach ($order_goods_list as $order_goods) {
                                $order_goods_ids[] = $order_goods['order_id'];
                            }
                            $where .= ' AND a.order_id in (' . implode(',', $order_goods_ids) . ")";
                        }
                }
            }

            $total = $order_model->query("SELECT COUNT(*) as count FROM {$order_model->table_name} AS a INNER JOIN {$GLOBALS['mysql']['MYSQL_DB_TABLE_PRE']}order_consignee AS b ON a.order_id = b.order_id {$where}", $binds);
            if ($total[0]['count'] > 0) {
                $limit = $order_model->set_limit(array(request('page', 1), request('pernum', 10)), $total[0]['count']);

                $sql = "SELECT a.id, a.order_id, a.order_status,a.payment_method,a.order_amount, a.created_date,
                ...
                $payment_method_map = $payment_method_model->payment_method_map();
                foreach ($list as &$v) {
                    $v['order_status'] = $status_map[$v['order_status']];
                    $v['payment_method'] = isset($payment_method_map[(int)$v['payment_method']]) ? $payment_method_map[(int)$v['payment_method']] : '';
    ...
    public function action_view()
    {
        ...
        if ($order = $order_model->find($condition)) {
            ...
            $order['payment_method_name'] = isset($payment_map[$order['payment_method']]['name']) ? $payment_map[$order['payment_method']]['name'] : "";
            ...
            foreach ($goods_list as &$goods) {
                if (!empty($goods['instead_goods_list'])) {
                    $simple_instead_goods_list = json_decode($goods['instead_goods_list'], true);
                    $goods_instead_ids = implode(',', array_column($simple_instead_goods_list, 'goods_instead_id'));
                    $sql = "select * from {$GLOBALS['mysql']['MYSQL_DB_TABLE_PRE']}goods_instead_sell where id in ({$goods_instead_ids})";
                    $complete_goods_instead_list = array_column($order_goods_model->query($sql), 'instead_sell_id', 'id');
                    foreach ($simple_instead_goods_list as &$simple_goods_instead) {
                        $simple_goods_instead['instead_sell_id'] = $complete_goods_instead_list[$simple_goods_instead['goods_instead_id']];
                    }
                    $goods['instead_goods_list'] = $simple_instead_goods_list;
                }
            }
            ...
            $this->refund_status_map = $order_goods_model::$refund_status_map;
            //日志列表
        ...
        if ($order = $order_model->find($condition)) {
            ...
            switch (request('step')) {
                ...
                case 'cancel': //取消交易
                    ...
                    if ($order['order_status'] == 1 || $_SESSION['ADMIN']['USER_ID'] == 1) {
                ...
                case 'shipping'://发货配送
                    if ($order['order_status'] == $order_model::ORDER_STATUS_WAIT_SHIPPING) {
                        $data = array
                        (
                            'order_id' => $order_id,
                        ...
                        if ($shipping_model->create($data) > 0) {
                            $order_goods_model = new order_goods_model();
                            $order_goods_model->shipping($order_id, request('goods_id_list', ''));
                            if (!$order_goods_model->find(array('order_id' => $order_id, 'is_shipping' => 0, 'refund_status' => 1))) {
                                    $order_model->update(['order_id' => $order_id], ['order_status' => $order_model::ORDER_STATUS_WAIT_RECEIVE]
                                );
                            }

                            //通知买家
                            if (request('info')) {
                                $this->send_shipping_message($order);
                            }
    ...
    public function send_shipping_message($order)
    {
        $memos = trim(request('memos', ''));
        $memos && $memos = "({$memos})";
        $condition = array('user_id' => $order['user_id']);
        $user_model = new user_model();
        ...
        kafka::produce(kafka::ORDER_SEND_OUT, [
            // 短信数据
            'message' => [
                'phoneNumber' => $user['mobile'],
                'tempId' => phone::SCENE_SHIPPING,
                'params' => $params
            ],
        ]);
    }
