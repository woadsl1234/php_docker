<?php

namespace app\model;

use app\dto\order_dto;
use app\exception\order_not_found_exception;
use plugin\push\wechat;

class order_model extends model
{
    public $table_name = 'order';

    private static $order_push = null;

    const ORDER_STATUS_CANCEL = 0;
    const ORDER_STATUS_WAIT_PAY = 1;
    const ORDER_STATUS_WAIT_SHIPPING = 2;
    const ORDER_STATUS_WAIT_RECEIVE = 3;
    const ORDER_STATUS_FINISH = 4;

    public $status_map = array
    (
        self::ORDER_STATUS_CANCEL => '交易取消',
        self::ORDER_STATUS_WAIT_PAY  => '等待付款',
        self::ORDER_STATUS_WAIT_SHIPPING  => '等待发货',
        self::ORDER_STATUS_WAIT_RECEIVE  => '等待签收',
        self::ORDER_STATUS_FINISH  => '交易完成'
    );

    const ORDER_TYPE_GENERAL = 1;
    const ORDER_TYPE_RECHARGE = 2;

    public $type_map = array
    (
        self::ORDER_TYPE_GENERAL => 'general',
        self::ORDER_TYPE_RECHARGE => 'recharge'
    );

    /**
     * 生成订单号
     */
    public function create_order_id()
    {
        $l = str_replace('.', '', sprintf("%.2f", ($_SERVER['REQUEST_TIME'] - $GLOBALS['verydows']['COMMENCED']) / 3600));
        $l = str_pad($l, 8, 0, STR_PAD_RIGHT) . date('s');
        $r = substr(microtime(), 2, 3). rand(0, 9) . rand(0, 9);
        $order_id = $l.$r;
        if($this->find(array('order_id' => $order_id))) return $this->create_order_id();
        return $order_id;
    }
    
    /**
     * 获取订单当前给用户的显示进度
     */
    public function get_user_order_progress($order_status, $payment_method)
    {
        switch($order_status)
        {
            case 0: $progress = array(1 => '提交订单', 0 => '交易取消'); break;
            
            case 1: 
                
                if($payment_method == 2)
                {
                    $progress = array(1 => '提交订单', 2 => '货到付款', 3 => '等待发货');
                }
                else
                {
                    $progress = array(1 => '提交订单', 2 => '等待付款');
                }
 
            break;
            
            case 2: 
            
                $progress = array(1 => '提交订单', 2 => '完成付款', 3 => '等待发货');
                if($payment_method == 2) $progress[2] = '货到付款';
            
            break;

            case 3:
            
                $progress = array(1 => '提交订单', 2 => '完成付款', 3=> '正在配送', 4 => '等待签收');
                if($payment_method == 2) $progress[2] = '货到付款';
                
            break;
            
            case 4: $progress = array(1 => '提交订单', 2 => '完成付款', 3 => '完成配送', 4 => '签收完成'); break;
            
            default: $progress = FALSE;
        }
        
        return $progress;
    }
    
    /**
     * 获取付款按钮操作
     */
    public function get_pay_btn_handle($order)
    {
        $payment_model = new payment_method_model();
        if($payment = $payment_model->find(array('id' => $order['payment_method'], 'enable' => 1)))
        {
            $gateway_obj = \plugin::instance('payment', $payment['pcode'], array($payment['params']));
            return $gateway_obj->create_pay_url($order);
        }
        return FALSE;
    }
    
    /**
     * 订单是否超期未付款
     * @return 超期自动取消返回FALSE 未超期返回剩余时间
     */
    public function is_overdue($order_id, $created_date)
    {
        $expire = $created_date + ($GLOBALS['cfg']['order_cancel_expires'] * 3600);
        if($expire <= $_SERVER['REQUEST_TIME'])
        {
            $this->update(array('order_id' => $order_id), array('order_status' => 0));
            return FALSE;
        }
        return $expire - $_SERVER['REQUEST_TIME'];
    }
    
    
    /**
     * 3小时过期订单批量处理
     */
    public function expired()
    {
        $expire = $_SERVER['REQUEST_TIME'] - ($GLOBALS['cfg']['order_cancel_expires'] * 3600);
        $status= self::ORDER_STATUS_WAIT_PAY;
        $this->start_transaction();
        $sql = "SELECT order_id FROM {$this->table_name}
                WHERE order_status = {$status} AND created_date <= {$expire}";
        if($outdated = $this->query($sql))
        {
            $order_goods_model = new order_goods_model();
            foreach($outdated as $v)
            {
                if($this->update(array('order_id' => $v['order_id']), array('order_status' => 0))) $order_goods_model->restocking($v['order_id']);
            }
        }
        $this->commit();
    }

    /**
     * @param $oauth_keys
     * @param $template_id
     * @param $url
     * @param $data
     */
    public function send_notice($oauth_keys, $template_id, $url, $data)
    {
        if (!self::$order_push) {
            self::$order_push = \plugin::instance('push', 'wechat');
        }

        self::$order_push->doSend($template_id, $oauth_keys, $data, $url);
    }

    public function get_valid_order_count($user_id)
    {
        $user_id = (int)$user_id;
        $sql = "select count(*) as amount from {$this->table_name}
                where user_id = {$user_id} and order_status >= 2";
        $res =  $this->query($sql);
        return $res[0]['amount'];
    }

    /**
     * 获取订单
     * @param $order_id
     * @throws order_not_found_exception
     * @return order_dto
     */
    public function get($order_id)
    {
        $order = $this->find(array('order_id' => $order_id));
        if (empty($order)) {
            throw new order_not_found_exception;
        }

        return new order_dto($order);
    }

    /**
     * 取消订单
     */
    public function cancel($order_id)
    {
        $this->update([
            'order_id' => $order_id
        ], ['order_status' => self::ORDER_STATUS_CANCEL]);
    }

    /**
     * 订单全部发货
     */
    public function shipping($order_id)
    {
        $this->update([
            'order_id' => $order_id
        ], ['order_status' => self::ORDER_STATUS_WAIT_RECEIVE]);
    }

    public function get_new_order_template($order_id, $pcode)
    {
        $condition = array('order_id' => $order_id);
        $order_model = new order_model();
        $order = $order_model->find($condition);
        $payment_method_model = new payment_method_model();

        //获取支付方式
        $payment_method_all = $payment_method_model->find_all(null, null, "pcode, id, name");
        $payment_method_name = "未知方式";
        foreach ($payment_method_all as $payment_method) {
            if ($payment_method['pcode'] == $pcode) {
                $payment_method_name = $payment_method['name'];
                break;
            }
        }

        //获取收货信息
        $consignee_model = new order_consignee_model();
        $consignee_row = $consignee_model->find($condition);
        $consignee = $consignee_row['receiver'] . " " . $consignee_row['address'] . $consignee_row['room'] . " " . $consignee_row['mobile'];

        //商品列表
        $order_goods_model = new order_goods_model();
        $goods_list = $order_goods_model->get_goods_list($order_id);
        $goods_name_list = implode("；", array_column($goods_list, 'goods_name'));

        $wechat_templates_model = new wechat_templates_model();
        $template_id = $wechat_templates_model->get_template_id("business_new_order_info");//订单通知模板
        $remark = "";
        trim($order['memos']) && $remark .= "备注：" . $order['memos'];

        //发送的数据
        $send_data = array(
            'first' => array('value' => "报告老板，您有一笔新订单！\n"),
            'keyword1' => array('value' => $order_id),
            'keyword2' => array('value' => $goods_name_list),
            'keyword3' => array('value' => "¥" . $order['order_amount']),
            'keyword4' => array('value' => $payment_method_name),
            'keyword5' => array('value' => $consignee),
            'remark' => array('value' => $remark),
        );

        //获取管理员列表
        $admin_model = new admin_model();
        $admin_list = $admin_model->find_all(null, null, "openid");
        $admin_openid_list = array_unique(array_column($admin_list, 'openid'));
        $tousers = [];
        foreach ($admin_openid_list as $touser) {
            $touser = trim($touser);
            if (strlen($touser) != 28) {
                continue;
            }
            $tousers[] = $touser;
        }

        $wechat = new wechat();
        return [
            'tousers' => $tousers,
            'template_id' => $template_id,
            'data' => $send_data,
            'access_token' => $wechat->get_access_token()
        ];
    }
}